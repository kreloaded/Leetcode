...
class Solution {
    ...
    private boolean existPath(TreeNode root, int arr[], int index) 
    { 
        if(root==null) { 
            return arr.length==0; 
        } if(index == arr.length - 1 && root.left == null && root.right == null && root.val == arr[index]) {
            return true;
        } if(index < arr.length && root.val == arr[index]) {
            return existPath(root.left, arr, index+1) || existPath(root.right, arr, index+1);
        }
        return false;
    } 
} 